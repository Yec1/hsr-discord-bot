...
describe("getBoundUserIds", () => {
	it("finds canonical and legacy bindings without treating UID records as users", () => {
		expect(
			getBoundUserIds([
				{
					id: "user-new",
					value: { hoyolabs: [{ characters: [{ uid: "800000001" }] }] }
				},
				{ id: "user-old", value: { account: [{ uid: "800000002" }] } },
				{ id: "800000001", value: { anomalyRankIcon: [{}] } },
				{ id: "empty", value: { hoyolabs: [] } }
			])
		).toEqual(["user-new", "user-old"]);
	});
});

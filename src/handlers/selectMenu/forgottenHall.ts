...
export async function handleForgottenHall(
	...
	const drawTask = async () => {
		try {
			...
			const result = await withUserHSRRequest(
				{ interaction, tr, userId: userId || "", accountIndex: 0 },
				async hsr => ({
					res: await hsr.record.forgottenHall(
						Number.parseInt(mode || "0"),

            ...
            await interaction.editReply({ embeds: [
                new EmbedBuilder()
                    ...
                    .addField(
                        ...
                        `**▶ ${tr('server_Name')}: \`${interaction.guild.name}\`
                            ▶ ID: \`${interaction.guild.id}\`
                            ▶ ${tr('server_Owner')}: <@${interaction.guild.ownerId}>
                            ▶ ${tr('server_Region')}: \`${regions[interaction.guild.region] || tr('server_Unknown_Region')}\`
                            ▶ ${tr('server_Tier')}: \`${interaction.guild.premiumTier ? `${tr('server_Tier_Lvl')} ${interaction.guild.premiumTier}` : tr('none')}\`
                            ▶ ${tr('server_Filter_Lvl')}: \`${filterLevels[interaction.guild.explicitContentFilter]}\`
                            ▶ ${tr('server_Verification_Lvl')}: \`${verificationLevels[interaction.guild.verificationLevel]}\`
                            ▶ ${tr('server_Time_Created')}: <t:${moment(interaction.guild.createdAt).unix()}:F> <t:${moment(interaction.guild.createdAt).unix()}:R>
                        **`, 
                        false
                    )
                    .addField(
                        ...
                        `**▶ ${tr('server_Role_Count')}: \`${roles.length}\`
                            ...
                            > ${tr('Regular_Emoji')}: \`${emojis.filter(emoji => !emoji.animated).size}\`
                            > ${tr('Animated_Emoji')}: \`${emojis.filter(emoji => emoji.animated).size}\`
                            ▶ ${tr('server_Member_Count')}: \`${interaction.guild.memberCount}\`
                            > ${tr('server_Member_Human')}: \`${members.filter(member => !member.user.bot).size}\`
                            > ${tr('server_Member_Bot')}: \`${members.filter(member => member.user.bot).size}\`
                            ▶ ${tr('server_Channel_Count')}: \`${text + voice}\`
                            > ${tr('server_Text_Channel')}: \`${text}\`
                            > ${tr('server_Voice_Channel')}: \`${voice}\`
                            ▶ ${tr('server_Boost_Count')}: \`${interaction.guild.premiumSubscriptionCount || '0'}\`
                        **`, 
                        false
                    )
                    // .addField(

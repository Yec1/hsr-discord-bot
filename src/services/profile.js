...
async function mainPage(playerData, interaction) {
	...
	for (let i = 0; i < playerData.characters.length; i++) {
		...
		ctx.quadraticCurveTo(x + Width, y + Height, x + Width, y + Height - 20);
...
async function charPage(characters, playerData, num, interaction) {
	...
	// Element
	const element = await loadImageAsync(image_Header + character.element.icon);
	ctx.drawImage(
		element,
	...
	const star = await loadImageAsync(
		image_Header + `icon/deco/Star${character.rarity == 5 ? "5" : "4"}.png`
	);
	...
	const attributesWithAdditions = allAttributes.reduce((acc, attribute) => {
		if (acc[attribute.field]) acc[attribute.field].value += attribute.value;
	...
	for (let i = 0; i < relics.length; i++) {
		...
		const relicImage = await loadImageAsync(image_Header + relics[i].icon);
		...
		for (let j = 0; j < relics[i].sub_affix.length; j++) {
			...
			ctx.drawImage(subAff, x + 103, y + affixYStart + j * 32, 32, 32);

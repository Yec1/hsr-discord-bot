import { client, database, commands } from "@/index.js";
import {
...
client.on(Events.InteractionCreate, async (interaction: Interaction) => {
	...
	if (interaction.isCommand()) {
		...
		try {
			...
			if (
				(interaction.member?.roles as any)?.cache?.has(
					"1012968415964704768"
				) &&
				!(await database.has(`${interaction.user.id}.premium`))
			)
				await database.set(`${interaction.user.id}.premium`, true);
				...
				embeds: [
					new EmbedBuilder()
						...
						.addFields({
							...
							value: `${
								((interaction as any).options as any)
									?._subcommand
									? `> ${((interaction as any).options as any)._subcommand}`
									: "\u200b"
							}`,
							inline: true
						})
			...
			console.error(e);
			new Logger("指令").error(`錯誤訊息：${(e as Error).message}`);
		...
		const command = commands.slash.get(contextMenuInteraction.commandName);
		if (!command) return;
		try {
			await command.execute(contextMenuInteraction);
		} catch (e) {
			new Logger("指令").error(`錯誤訊息：${(e as Error).message}`);
			await replyOrfollowUp(interaction, {
				content: "哦喲，好像出了一點小問題，請重試",

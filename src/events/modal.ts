...
async function handleNewCookieSet(
	...
	const cookie = `ltoken_v2=${ltoken_v2}; ltuid_v2=${ltuid_v2}; cookie_token_v2=${cookie_token_v2}; account_mid_v2=${account_mid_v2}; ltmid_v2=${account_mid_v2};`;
...
async function handlePasswordLogin(
	...
	if (result.status === "success" && result.cookies) {
		...
		// Listen for result
		VerificationServer.onResult(sessionId, async (verificationResult: any) => {
			...
			// The prefix is session_id, NOT mmt_type
			// mmt_data is the result fields directly (no outer wrapper)
			...
			// Retry login with verification result, preserving the original device_id
		...
		if (sendResult.status === "error") {
			if (sessionId) sessionStatuses.set(sessionId, { status: "error", message: `驗證碼發送失敗：${sendResult.message}` });
			await interaction.editReply({
				content: null, components: [],
				embeds: [
					new EmbedBuilder()
						.setTitle(tr("account_LoginFailed"))
						.setDescription(`驗證碼發送失敗：${sendResult.message}`)
						.setColor("#E76161")
				]
			});
			return;
		...
		if (sessionId) {
			...
			const webUrl =
				(config as any).WEBSITE_URL ||
				`http://localhost:${config.WEBSERVER_PORT || 3000}`;
			const emailVerifyUrl = `${webUrl}/hsr/email-verify?session=${emailSessionId}`;

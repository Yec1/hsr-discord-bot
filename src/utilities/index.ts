...
interface UserHsrOptions {
	suppressErrorReply?: boolean | undefined;
}

export interface UserHsrContext extends UserHsrOptions {
	interaction: Interaction;
...
function resolveHsrLang(
	...
	if (interaction && "locale" in interaction) {
		const locale = (interaction as any).locale;
		if (locale === "zh-TW") return LanguageEnum.TRADIIONAL_CHINESE;
		if (locale === "zh-CN") return LanguageEnum.SIMPLIFIED_CHINESE;
	}
	return LanguageEnum.ENGLISH;
}

function isCookieAuthError(error: any): boolean {
	const code =
		error instanceof HoyoAPIError
			? error.code
			: (error?.code ?? error?.retcode);
...
function reportHsrError(
	...
	if (context.suppressErrorReply) return;
...
export async function getUserHSRData(
	...
	const lang = resolveHsrLang(userLang, interaction);

	if (!cookie || !uid) {
		if (!options?.suppressErrorReply) {
			checkAccount(interaction, tr, userId, {
				hasCookie: Boolean(cookie),
				Lang: lang,
				hasUid: Boolean(uid)
			});
...
export async function withUserHSRRequest<T>(
	...
	refreshCookie: typeof autoRefreshCookie = autoRefreshCookie
): Promise<T | null> {
	const [cookie, userLang, uid] = await Promise.all([
		getUserCookie(context.userId, context.accountIndex),
		getUserLang(context.userId),
		getUserUid(context.userId, context.accountIndex)
	]);
	const lang = resolveHsrLang(userLang, context.interaction);
	const hsr = await getUserHSRData(
		context.interaction,
		context.tr,
		context.userId,
		context.accountIndex,
		{ suppressErrorReply: context.suppressErrorReply }
	);
	if (!hsr) return null;

	try {
		return await operation(hsr);
	} catch (error: any) {
		if (cookie && isCookieAuthError(error)) {
			...
			if (refreshed.success) {
				const retryCookie =
					refreshed.newCookie ||
					(await getUserCookie(context.userId, context.accountIndex));
				if (retryCookie && uid) {
					...
					try {
						return await operation(retryHsr);
		...
		reportHsrError(context, error, cookie, uid, lang);
		return null;
	}
}
